...
- 工具栏集中放置导出、里程碑和日期计算器入口；导出页支持返回工具栏。
- 里程碑：显示当前年份与当前月份进度，可新增仅在里程碑页可见的阶段节点；列表支持长按拖动排序，长按卡片也可进入管理态并高亮当前项，手动勾选后通过底部图标工具栏导出或删除，单卡片不显示删除按钮，导出前支持再次拖动排序。
...
- 长按应用图标可请求添加小组件。
- “关于”页主动检查 GitHub Release 更新：优先 latest.json，失败后使用 GitHub Releases API。
- 用户事件、分类、备注和图片不会上传。
